...
function categorizeCard(card) {
    const content = (card.q + ' ' + card.a + ' ' + (card.explanation || '')).toLowerCase();
    
    // Definir categorias baseado em palavras-chave
    if (content.includes('permutação') || content.includes('anagrama') || content.includes('ordenar') || content.includes('p(')) {
        return 'permutacao';
    } else if (content.includes('arranjo') || content.includes('a(') || content.includes('presidente') || content.includes('vice')) {
        return 'arranjo';
    } else if (content.includes('combinação') || content.includes('c(') || content.includes('subconjunto') || content.includes('escolher')) {
        return 'combinacao';
    } else if (content.includes('princípio') || content.includes('produto cartesiano') || content.includes('pares ordenados')) {
        return 'principios';
    } else {
        return 'geral';
    }
...
function getDifficulty(card) {
    ...
    if (card.type === 'formula' || content.includes('fórmula') || content.includes('definição')) {
        return 'basico';
    }
    
    // Intermediário: cálculos mais complexos, exemplos detalhados
    if (content.includes('banana') || content.includes('circular') || content.includes('repetição') || 
        content.includes('combinação com repetição') || content.match(/\d{2,}/)) {
...
fs.writeFileSync('cards_with_metadata.json', JSON.stringify(processedCards, null, 4), 'utf8');

console.log(`✅ Processados ${processedCards.length} cards com metadados`);
console.log('📊 Categorias encontradas:', [...new Set(processedCards.map(c => c.category))]);
console.log('📈 Dificuldades:', [...new Set(processedCards.map(c => c.difficulty))]);
